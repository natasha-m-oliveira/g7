...
if (!empty($_SESSION["username"]) && $_SESSION["access"] = 4) {
    ...
    if(!empty($_POST["delete-btn"])){
        $responseDelete = $event->deleteEvent();
    }
} else {
    $url = "../index.php";
    header("Location: $url");
...
<body>
    <div class="content">
        <main>
            <div class="container">
                <div class="form">
                    <form name="event-id" action="" method="post" class="delete" onsubmit="return true">
                        <h1>Excluir Evento</h1>
                        <input type="text" name="event-id" id="event-id" class="sr-only" value="<?php echo $_GET['id']; ?>" readonly>
                        <div class="toast">
                            <?php
